...
class NonconvexOptimization(object):
    ...
    def add_bounded_constraint(self, lbg, g, ubg):
        ...
        if lbg is not None:
            ...
            self._ubg_list += [
                np.infty * np.ones(shape=(g.shape[0], g.shape[1]))
            ]
    ...
    def reshape_lists(self):
        ...
        for ix in range(len(self._g_list)):
            if self._g_list[ix].shape[1] != 1:
                ...
                tall_lbg_list += [
                    cas.reshape(self._lbg_list[ix], (tall_dim, 1))
                ]
                tall_ubg_list += [
                    cas.reshape(self._ubg_list[ix], (tall_dim, 1))
                ]
            else:
...
class MultiMPC(NonconvexOptimization):
    ...
    def __init__(self,
        ...
        self.x_ctrld = [
            cas.MX.sym('x_ctrl%02d' % i, n_state, N + 1)
            for i in range(self.n_vehs_cntrld)
        ]
        self.u_ctrld = [
            cas.MX.sym('u_ctrl%02d' % i, n_ctrl, N)
            for i in range(self.n_vehs_cntrld)
        ]
        self.xd_ctrld = [
            cas.MX.sym('xd_ctrl%02d' % i, n_desired, N + 1)
            for i in range(self.n_vehs_cntrld)
        ]

        # Declare the slack variables
        if self.n_other_vehicle > 0:
            self.slack_i_jnc = cas.MX.sym('s_i_jnc', self.n_other_vehicle,
                                          N + 1)
            self.slack_ic_jnc = [
                cas.MX.sym('s_i%02d_jnc' % i, self.n_other_vehicle, N + 1)
                for i in range(self.n_vehs_cntrld)
            ]
        else:
            self.slack_i_jnc = 0
        ...
        if self.n_vehs_cntrld > 0:
            ...
            self.slack_ic_jc = [
                cas.MX.sym("s_ic%02d_jc" % ic, self.n_vehs_cntrld, N + 1)
        ...
        self.top_wall_slack_c = [
            cas.MX.sym('sc_top_%02d' % i, 1, N + 1)
            for i in range(self.n_vehs_cntrld)
        ]
        self.bottom_wall_slack_c = [
            cas.MX.sym('sc_bot_%02d' % i, 1, N + 1)
            for i in range(self.n_vehs_cntrld)
        ]

        self._x_list = mpcx_to_nlpx(
            self.n_other_vehicle, self.x_ego, self.u_ego, self.xd_ego,
            self.x_ctrld, self.u_ctrld, self.xd_ctrld, self.slack_i_jnc,
            self.slack_ic_jnc, self.slack_i_jc, self.slack_ic_jc,
            self.top_wall_slack, self.bottom_wall_slack, self.top_wall_slack_c,
            self.bottom_wall_slack_c)

        # Parameters
        self.x0_ego = cas.MX.sym('x0_ego', n_state, 1)
        self.p_ego = VehicleParameters(self.n_vehs_cntrld,
                                       self.n_other_vehicle, "ego")
        ...
        self.p_cntrld_list = [
            VehicleParameters(self.n_vehs_cntrld, self.n_other_vehicle,
                              "ctrl%02d" % i)
            for i in range(self.n_vehs_cntrld)
        ]
        ...
        self.total_svo_cost = self.compute_mpc_costs(
            self.n_vehs_cntrld, self.n_other_vehicle, self.x_ego, self.u_ego,
            self.xd_ego, self.p_ego, self.p_theta_ic, self.p_theta_i_ego,
            self.x_ctrld, self.u_ctrld, self.xd_ctrld, self.p_cntrld_list,
            self.x_other, self.p_other_vehicle_list, self.slack_i_jnc,
            self.slack_ic_jnc, self.slack_i_jc, self.slack_ic_jc,
            self.top_wall_slack, self.bottom_wall_slack, self.top_wall_slack_c,
            self.bottom_wall_slack_c, self.x0_ego, self.x0_cntrld,
            self.x0_allother, params, self.k_ca2, self.k_CA_power,
            self.k_slack, self.k_CA)
        ...
        self.add_mpc_constraints(
            ...
            self.p_other_vehicle_list, self.dt, self.slack_i_jnc,
            self.slack_ic_jnc, self.slack_i_jc, self.slack_ic_jc,
            self.top_wall_slack, self.bottom_wall_slack, self.top_wall_slack_c,
            self.bottom_wall_slack_c, params)
        # Total optimization costs
        self.solver, self.solver_prefix = self.get_nlpsol(ipopt_params)

    def add_vehicle_constraints(self, world, p_ego, x_ego, u_ego, xd_ego,
                                x0_ego, dt):
        ego_lane_number = p_ego.desired_lane
        fd = self.gen_f_desired_lane(
            world, ego_lane_number,
            right_direction=True)  # TODO:  This could mess things up
        f = self.gen_f_vehicle_dynamics(p_ego, model="kinematic_bicycle")
        self.add_dynamics_constraints_g(x_ego, u_ego, xd_ego, x0_ego, f, fd,
                                        dt)
        self.add_state_constraints_g(x_ego, u_ego, p_ego)

    def add_mpc_constraints(self, N, n_vehs_cntrld, n_other_vehicle, world,
                            x_ego, u_ego, xd_ego, x0_ego, p_ego, x_ctrld,
                            u_ctrld, xd_ctrld, x0_ctrld, p_ctrld_list, x_other,
                            x0_allother, p_other_vehicle_list, dt, slack_i_jnc,
                            slack_ic_jnc, slack_i_jc, slack_ic_jc,
                            top_wall_slack, bottom_wall_slack,
        ...
        '''
        self.add_vehicle_constraints(world, p_ego, x_ego, u_ego, xd_ego,
                                     x0_ego, dt)

        for j in range(n_vehs_cntrld):
            self.add_vehicle_constraints(world, p_ctrld_list[j], x_ctrld[j],
                                         u_ctrld[j], xd_ctrld[j], x0_ctrld[j],
                                         dt)

        self.constrain_slack_positive(n_vehs_cntrld, n_other_vehicle,
                                      slack_i_jc, slack_i_jnc, slack_ic_jc,
                                      slack_ic_jnc, top_wall_slack,
                                      bottom_wall_slack, top_wall_slack_c,
                                      bottom_wall_slack_c)
        ...
        for k in range(N + 1):
            ...
            for i in range(n_other_vehicle):
                ...
                self.add_bounded_constraint(
                    ((1 - slack_i_jnc[i, k]) * within_collision_distance),
                    dist, None)

            for j in range(n_vehs_cntrld):
                dist = minkowski_ellipse_collision_distance(
                    p_ego, p_ctrld_list[j], x_ego[0, k], x_ego[1, k], x_ego[2,
                                                                            k],
        ...
        for ic in range(self.n_vehs_cntrld):
            for k in range(N + 1):
                ...
                for j in range(self.n_other_vehicle):
                    ...
                    self.add_bounded_constraint(((1 - slack_ic_jnc[ic][j, k]) *
                                                 within_collision_distance),
                                                dist, None)
                ...
                for j in range(n_vehs_cntrld):
                    ...
                    if j <= ic:
                        self.add_equal_constraint(slack_ic_jc[ic][j, k],
                                                  np.zeros((1, 1)))
                    else:
        ...
        if self.safety_constraint:
            ...
            if len(x_ctrld) > 0:
                self.generate_circles_stopping_constraint(
                    x_ego, x_ctrld, p_ego.L, p_ego.W, max_deceleration)

                for jc in range(len(x_ctrld)):
                    self.generate_circles_stopping_constraint(
                        x_ctrld[jc], x_ctrld[:jc] + x_ctrld[jc + 1:] + x_other,
                        p_ego.L, self.p_ego.W, max_deceleration)

            if len(self.x_other) > 0:
                self.generate_circles_stopping_constraint(
                    x_ego, x_other, p_ego.L, p_ego.W, max_deceleration)

    def constrain_slack_positive(self, n_vehs_cntrld, n_other_vehicle,
                                 slack_i_jc, slack_i_jnc, slack_ic_jc,
                                 slack_ic_jnc, top_wall_slack,
                                 bottom_wall_slack, top_wall_slack_c,
                                 bottom_wall_slack_c):
        ...
        if n_other_vehicle > 0:
            self.add_bounded_constraint(np.zeros(shape=slack_i_jnc.shape),
                                        slack_i_jnc, None)
            for slack_var in slack_ic_jnc:
                self.add_bounded_constraint(np.zeros(shape=slack_var.shape),
                                            slack_var, None)

        # Slack variables related to cntrld vehicles
        if n_vehs_cntrld > 0:
            self.add_bounded_constraint(np.zeros(shape=slack_i_jc.shape),
                                        slack_i_jc, None)
            for ic in range(len(slack_ic_jc)):
                self.add_bounded_constraint(
                    np.zeros(shape=slack_ic_jc[ic].shape), slack_ic_jc[ic],
                    None)

        for ic in range(len(top_wall_slack_c)):
            for k in range(top_wall_slack_c[ic].shape[1]):
                self.add_bounded_constraint(0, top_wall_slack_c[ic][0, k],
                                            None)
                self.add_bounded_constraint(0, bottom_wall_slack_c[ic][0, k],
                                            None)
    ...
    def compute_mpc_costs(self, n_vehs_cntrld, n_other_vehicle, x_ego, u_ego,
                          xd_ego, p_ego, p_theta_ic, p_theta_i_ego, x_ctrld,
                          u_ctrld, xd_ctrld, p_cntrld_list, x_other,
                          p_other_vehicle_list, slack_i_jnc, slack_ic_jnc,
                          slack_i_jc, slack_ic_jc, top_wall_slack,
                          bottom_wall_slack, top_wall_slack_c,
                          bottom_wall_slack_c, x0_ego, x0_cntrld, x0_allother,
                          params, k_ca2, k_CA_power, k_slack, k_CA):
        ...
        response_costs, _ = self.generate_veh_costs(x_ego, u_ego, xd_ego,
                                                    p_ego)
        ...
        for idx in range(n_vehs_cntrld):
            ...
            nonresponse_cost, _ = self.generate_veh_costs(
                x_ctrld[idx], u_ctrld[idx], xd_ctrld[idx], p_cntrld_list[idx])
        ...
        slack_cost = self.compute_quadratic_slack_cost(n_other_vehicle,
                                                       n_vehs_cntrld,
                                                       slack_i_jnc,
                                                       slack_ic_jnc,
        ...
        total_svo_cost = (response_svo_cost + other_svo_cost +
                          k_slack * slack_cost + k_CA * collision_cost)
        return total_svo_cost

    def compute_quadratic_slack_cost(self, n_other_vehicle, n_vehs_cntrld,
                                     slack_i_jnc, slack_ic_jnc, slack_i_jc,
                                     slack_ic_jc):
        slack_cost = 0
    ...
    def add_state_constraints_g(self,
                                ...
                                U,
                                ego_veh,
                                strict_wall_y_constraint=False):
        ...
        for k in range(U.shape[1]):
            self.add_bounded_constraint(-ego_veh.max_delta_u, U[0, k],
                                        ego_veh.max_delta_u)
            self.add_bounded_constraint(ego_veh.min_v_u, U[1, k],
                                        ego_veh.max_v_u)  # 0-60 around 4 m/s^2

    def add_dynamics_constraints_g(self, X, U, X_desired, x0, f, fd,
                                   dt: float):
        N = U.shape[1]
        for k in range(N):
            self.add_equal_constraint(X[:, k + 1],
                                      self.F_kutta(f, X[:, k], U[:, k], dt))
    ...
    def gen_f_vehicle_dynamics(self, p_veh, model: str = "kinematic_bicycle"):
        ...
        if model == "kinematic_bicycle":
            ...
            ode = cas.vertcat(V * cas.cos(Phi), V * cas.sin(Phi),
                              V * cas.tan(Delta) / p_veh.L, delta_u, v_u, V)

            f = cas.Function('f', [x, u], [ode], ['x', 'u'], ['ode'])
        else:
            raise Exception("Have not implemented non-kinematic bicycle: %s" %
                            model)
        return f
    ...
    def generate_veh_costs(self, X, U, X_desired, p_car):
        ...
        phidot_cost = self.generate_phidot_cost(
            X, p_car)  #this function assumes a kinematic bicycle model
        N = U.shape[1]
        ...
        all_costs = [
            p_car.k_u_delta * u_delta_cost, p_car.k_u_v * u_v_cost,
            p_car.k_lat * lat_cost, p_car.k_lon * lon_cost,
            p_car.k_phi_error * phi_error_cost, p_car.k_phi_dot * phidot_cost,
            p_car.k_s * s_cost, p_car.k_v * v_cost,
            p_car.k_change_u_v * change_u_v,
            p_car.k_change_u_delta * change_u_delta,
            p_car.k_final * final_costs, p_car.k_x * x_cost,
            p_car.k_x_dot * x_dot_cost
        ]
    ...
    def generate_lateral_cost(self, X, X_desired):
        ...
        lateral_cost = np.sum([
            (-cas.sin(X_desired[2, k]) * (X[0, k] - X_desired[0, k]) +
    ...
    def generate_longitudinal_cost(self, X, X_desired):
        ...
        longitudinal_cost = np.sum([
            (cas.cos(X_desired[2, k]) * (X[0, k] - X_desired[0, k]) +
    ...
    def get_bestresponse_solution(self):
        x1, u1, x1_des, = (self.solution.value(self.x_ego),
    ...
    def get_solution(self):
        ...
        cntrld_x = [
            self.solution.value(self.x_ctrld[i])
            for i in range(len(self.vehs_ctrld))
        ]
        cntrld_u = [
            self.solution.value(self.u_ctrld[i])
            for i in range(len(self.vehs_ctrld))
        ]
        cntrld_des = [
            self.solution.value(self.xd_ctrld[i])
            for i in range(len(self.vehs_ctrld))
        ]

        other_x = [
            self.solution.value(self.x_other[i])
            for i in range(len(self.other_vehicle_list))
        ]
        other_u = [
            self.solution.value(self.u_other[i])
            for i in range(len(self.other_vehicle_list))
        ]
        other_des = [
            self.solution.value(self.xd_other[i])
            for i in range(len(self.other_vehicle_list))
        ]

        return x1, u1, x1_des, cntrld_x, cntrld_u, cntrld_des, other_x, other_u, other_des

    def generate_collision_ellipse(self, x_e, y_e, x_o, y_o, phi_o, alpha_o,
                                   beta_o):
        ...
        M = cas.vertcat(cas.horzcat(1 / alpha_o**2, 0),
                        cas.horzcat(0, 1 / beta_o**2))
        dX = cas.vertcat(dx, dy)
        prod = cas.mtimes([dX.T, R_o.T, M, R_o, dX])

        M_smaller = cas.vertcat(cas.horzcat(1 / (0.5 * alpha_o)**2, 0),
                                cas.horzcat(0, 1 / (0.5 * beta_o)**2))
        dist_prod = cas.mtimes([dX.T, R_o.T, M_smaller, R_o, dX])

        return dist_prod, prod

    def compute_wall_slack_costs(self, N, n_vehs_cntrld, top_wall_slack,
                                 bottom_wall_slack, top_wall_slack_c,
                                 bottom_wall_slack_c, params):
        slack_cost = 0
        for k in range(N + 1):
            slack_cost += (top_wall_slack[0, k]**2 +
                           bottom_wall_slack[0, k]**2)

        for ic in range(n_vehs_cntrld):
            for k in range(N + 1):
                slack_cost += (top_wall_slack_c[ic][0, k]**2 +
                               bottom_wall_slack_c[ic][0, k]**2)

        return slack_cost

    def compute_collision_avoidance_costs(self, N, n_other_vehicle,
                                          n_vehs_cntrld, x0_ego, x0_allother,
                                          x0_cntrld, p_ego,
                                          p_other_vehicle_list, p_cntrld_list,
                                          x_ego, x_other, x_ctrld, params,
                                          k_ca2, k_CA_power):
        ''' n_other_vehicles
                x0_all_other
                x0_ego
        ...
        for k in range(N + 1):
            ...
            for j in range(n_vehs_cntrld):
                dist = minkowski_ellipse_collision_distance(
                    p_ego, p_cntrld_list[j], x_ego[0, k], x_ego[1, k],
                    x_ego[2, k], x_ctrld[j][0, k], x_ctrld[j][1, k],
                    x_ctrld[j][2, k])
                distance_clipped = cas.fmax(
                    dist, k_ca2 +
                    0.001)  # This can be a smaller distance if we'd like
                ...
                for j in range(n_vehs_cntrld):
                    if j <= ic:
                        ...
                        dist = minkowski_ellipse_collision_distance(
                            ...
                            x_ctrld[ic][2, k], x_ctrld[j][0, k],
                            x_ctrld[j][1, k], x_ctrld[j][2, k])
                        distance_clipped = cas.fmax(
                            dist, k_ca2 + 0.001
    ...
    def plot_collision_slack_cost(self):
        ...
        for i in range(len(self.x_other)):
            ...
            R_o = np.array([[np.cos(phi_o), np.sin(phi_o)],
                            [-np.sin(phi_o), np.cos(phi_o)]])
    ...
    def generate_corners_stopping_constraint(self,
        ...
        for k in range(N):
            ...
            for j in range(len(x_others)):
                ...
                v_j = cas.vertcat(xj[4] * cas.cos(xj[2]),
                                  xj[4] * cas.sin(xj[2]))
                ...
                for xy_i in ego_corners:
                    for xy_j in ado_corners:
                        ...
                        self.add_bounded_constraint(
                            ...
                            cas.sqrt(2 * (alpha_i + alpha_j) *
                                     (rel_dist_mag - min_buffer_distance)))
            ...
            for j in range(len(x_others)):
                ...
                for xy_i in ego_centers:
                    for xy_j in ado_centers:
                        ...
                        stopping_distance = cas.fmax(
                            rel_dist_mag - min_distance, 0.00001)
                        self.add_bounded_constraint(min_distance**2,
                                                    delta_p_ij.T @ delta_p_ij,
                                                    None)
                        self.add_bounded_constraint(
                            0, delta_p_ij.T @ delta_v_ij / rel_dist_mag +
                            cas.sqrt(2 * (alpha_i + alpha_j) *
    ...
    def generate_stopping_constraint_ttc(self,
                                         x_opt,
                                         xcntrld_opt,
                                         xothers_opt,
        ...
        for k in range(N):
            ...
            phi_ego = x_opt[2, k]
            v_ego = x_opt[4, k]
            v_ego_components = (v_ego * cas.cos(phi_ego),
                                v_ego * cas.sin(phi_ego))
            ...
            for j in range(len(xcntrld_opt)):
                ...
                v_amb_components = (v_amb * cas.cos(phi_amb),
                                    v_amb * cas.sin(phi_amb))

                dxegoamb = (x_amb - x_ego) - car_length
                dyegoamb = (y_amb - y_ego) - car_width
                dot_product = (v_ego_components[0] - v_amb_components[0]
                               ) * dxegoamb + (v_ego_components[1] -
                                               v_amb_components[1]) * dyegoamb
                positive_dot_product = cas.max(
                    dot_product, 0
                )  # if negative, negative_dot_product will be 0 so time_to_collision is very very large

                time_to_collision = (dxegoamb**2 +
                                     dyegoamb**2) / (dot_product + 0.000001)
                ...
                self.add_bounded_constraint(
                    0, (dxegoamb**2 + dyegoamb**2) /
                    (0.000001 + min_time_to_collision) - dot_product)
            ...
            for j in range(len(xothers_opt)):
                ...
                dot_product = (v_ego_components[0] - v_j_components[0]
                               ) * dxego + (v_ego_components[1] -
                                            v_j_components[1]) * dyego
                positive_dot_product = cas.max(
                    dot_product, 0
                )  # if negative, negative_dot_product will be 0 so time_to_collision is very very large
                time_to_collision = (dxego**2 + dyego**2) / (
                    positive_dot_product + 0.000001)
                time_to_collision_cost += k_ttc * 1 / time_to_collision**2
                ...
                if add_constraint_for_cntrld:
                    for jc in range(len(xcntrld_opt)):
                        ...
                        dot_product = (v_ctrl_components[0] -
                                       v_j_components[0]) * dxctrl + (
                                           v_ctrl_components[1] -
                                           v_j_components[1]) * dyctrl
                        positive_dot_product = cas.max(
                            dot_product, 0
                        )  # if negative, negative_dot_product will be 0 so time_to_collision is very very large
                        time_to_collision = (dxctrl**2 + dyctrl**2) / (
                            positive_dot_product + 0.000001)
                        time_to_collision_cost += k_ttc * 1 / time_to_collision**2
                        self.add_bounded_constraint(
                            0, -dot_product + (dxctrl**2 + dyctrl**2) /
                            (0.00000001 + min_time_to_collision))
                        # self.opti.subject_to(dot_product <= (dxctrl**2 + dyctrl**2) /
                        #                      (0.00000001 + min_time_to_collision))

    def generate_stopping_constraint(self,
                                     x_opt,
                                     xamb_opt,
                                     xothers_opt,
                                     solve_amb,
        ...
        """
        u_v_maxbraking = (self.ego_veh.min_v_u
                          )  # this is max change in V, in discrete steps
        ...
        for k in range(N):
            ...
            if xamb_opt is not None:
                ...
                egobehind_amb = cas.fmax(
                    -dxegoamb, 0)  ## 0 if ego is beind, else should be >0

                v_max_constraint = cas.fmax(
                    (v_amb**2 - 2 * a_maxbraking *
                     (dist_egoamb - safety_buffer)),
                    999 * egobehind_amb,
                )
            ...
            for j in range(len(xothers_opt)):
                ...
                ego_behind = cas.fmax(
                    -dxego, 0)  ###how many meters behind or 0 if ahead/same
                v_max_constraint = cas.fmax(
                ...
                if xamb_opt is not None and solve_amb:
                    ...
                    amb_behind = cas.fmax(
                        -dxamb,
                        0)  ## 0 if ambulance is beind, else should be >0
                    v_max_constraint = cas.fmax(
                        (v_j**2 - 2 * a_maxbraking *
                         (dist_amb - safety_buffer)),
                        999 * amb_behind,
                    )
...
def save_costs(file_name, ibr):
    ...
    car1_costs_list = np.array(
        [ibr.opti.debug.value(cost) for cost in ibr.car1_costs_list])
    amb_costs_list = np.array(
        [ibr.opti.debug.value(cost) for cost in ibr.amb_costs_list])
...
def load_costs(file_name):
    ...
    car1_costs_list = np.load(file_name + "car1_costs_list.npy",
                              allow_pickle=False)
    amb_costs_list = np.load(file_name + "amb_costs_list.npy",
                             allow_pickle=False)
    svo_cost = np.load(file_name + "svo_cost.npy", allow_pickle=False)
    other_svo_cost = np.load(file_name + "other_svo_cost.npy",
                             allow_pickle=False)
    total_svo_cost = np.load(file_name + "total_svo_cost.npy",
                             allow_pickle=False)
...
def load_costs_int(i):
    ...
    car1_costs_list = np.load("%03dcar1_costs_list.npy" % i,
                              allow_pickle=False)
...
def get_vehicle_corners(x_state, length, width):
    ...
    corners = [(length / 2.0, width / 2.0), (-length / 2.0, width / 2.0),
...
def nlpp_to_mpcp(nlp_p,
    ...
    if p_size is None:
        raise Exception(
            "Parameter list size not inputed as argument. Make sure to provide p_size"
        )
    ...
    for _ in range(n_other_vehicles):
        x_other_nc.append(
            cas.reshape(nlp_p[idx:idx + n_state * (N + 1)], (n_state, N + 1)))
        idx += n_state * (N + 1)
...
def mpcx_to_nlpx(n_other: int, x_ego, u_ego, xd_ego, x_ctrl: List,
                 u_ctrl: List, xd_ctrl: List, s_i_jnc, s_ic_jnc, s_i_jc,
                 s_ic_jc, s_top, s_bottom, s_c_top, s_c_bottom):
    ...
    if n_ctrld_vehicles > 0:
        ...
        for i in range(n_ctrld_vehicles):
            long_list.append(
                cas.reshape(s_ic_jc[i], (n_ctrld_vehicles * (N + 1), 1)))
...
def nlpx_to_mpcx(nlp_x,
                 N: int = 0,
                 n_ctrld_vehicles: int = 0,
                 n_other: int = 0,
                 n_state=6,
                 n_ctrl=2,
                 n_desired=3):
    ...
    idx += n_ctrl * N
    xd_ego = cas.reshape(nlp_x[idx:idx + n_desired * (N + 1)],
                         (n_desired, N + 1))
    idx += n_desired * (N + 1)
    ...
    for i in range(n_ctrld_vehicles):
        x_ctrl.append(
            cas.reshape(nlp_x[idx:idx + n_state * (N + 1)], (n_state, N + 1)))
        idx += n_state * (N + 1)
        u_ctrl.append(cas.reshape(nlp_x[idx:idx + n_ctrl * N], (n_ctrl, N)))
        idx += n_ctrl * N
        xd_ctrl.append(
            cas.reshape(nlp_x[idx:idx + n_desired * (N + 1)],
                        (n_desired, N + 1)))
        idx += n_desired * (N + 1)
    ...
    s_ic_jnc = []
    if n_other > 0:
        s_i_jnc = cas.reshape(nlp_x[idx:idx + n_other * (N + 1)],
                              (n_other, N + 1))
        idx += n_other * (N + 1)

        for i in range(n_ctrld_vehicles):
            s_ic_jnc.append(
                cas.reshape(nlp_x[idx:idx + n_other * (N + 1)],
                            (n_other, (N + 1))))
            idx += n_other * (N + 1)
    ...
    s_ic_jc = []
    if n_ctrld_vehicles > 0:
        s_i_jc = cas.reshape(nlp_x[idx:idx + n_ctrld_vehicles * (N + 1)],
                             (n_ctrld_vehicles, N + 1))
        idx += n_ctrld_vehicles * (N + 1)

        for i in range(n_ctrld_vehicles):
            s_ic_jc.append(
                cas.reshape(nlp_x[idx:idx + n_ctrld_vehicles * (N + 1)],
                            (n_ctrld_vehicles, (N + 1))))
            idx += n_ctrld_vehicles * (N + 1)
