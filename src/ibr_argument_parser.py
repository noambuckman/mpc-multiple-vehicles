...
class IBRParser(ArgumentParser):
    def __init__(self):
        ...
        self.add_argument('--log-subdir', type=str, default=None, help="If you'd like to specify the log subdir name, end without /")
        self.add_argument('--n-processors', type=int, default=15, help="Number of processors used when solving a single mpc")

        ## Simulation Settings
        self.add_argument('--mpc-start-iteration', type=int, default=0, help="At which mpc iteration should the simulation start")
        ...
        self.add_argument('--svo-theta', type=float, default=0.0, help="Setting a homogeneous svo, random-svo MUST be set as 0")
        ...
        self.add_argument('--p-exec', type=float, default=0.4, help="Percent of control points executed at each iteration of MPC")

        ## IBR Settings
        self.add_argument('--n-ibr', type=int, default=3, help="Number of rounds of iterative best response before excuting mpc")
        ...
        self.add_argument('--k-solve-amb-max-ibr', type=int, default=2, help="Max number iterations where ado solves for ambulance controls, afterwards only ado")
        self.add_argument('--plan-fake-ambulance', action='store_true')
        self.add_argument('--save-ibr', type=int, default=1, help="Save the IBR control inputs, 1=True, 0=False")


        ## MPC Solver & Cost Settings
        self.add_argument('--default-n-warm-starts', type=int, default=15, help="Number of warm starts it will try when solving the MPC")
        self.add_argument('--k-max-slack', type=float, default=0.01, help="Maximum allowed collision slack/overlap between vehicles")
        self.add_argument('--k-max-solve-number', type=int, default=3, help = "Max re-solve attempts of the mpc for an individual vehicle")
        self.add_argument('--k-max-round-with-slack', type=int, default=np.infty, help = "Max rounds of ibr with slack variable used [SEMI DEPRECATED]")    
