...
if __name__ == "__main__":
    ...
    parser = IBRParser()
    parser.add_argument("history_file", type=str, help="load a history file")
    ...
    q_network = QNet(params["n_other"], 1, 25, .3, 3)
    q_network.double()
    q_network.to(device)
    if args.learning_agent == 0:
        dataset = SVODataset(params["history_file"], ambulance_only=True)
    else:
        dataset = SVODataset(params["history_file"], ambulance_only=False)
    ...
    for ep_tix in trange(params["train_epochs"]):
        for dataloader_type in dataloaders:
            ...
            for idx, batch in enumerate(dataloader):
                ...
                theta_ijx = batch["svos"]
                # TODO: rename V_ambx to V_learningagent
                V_ambx = batch["values"][:, args.learning_agent]
                batch_size = V_ambx.shape[0]
                theta_ijx = theta_ijx.to(device)
                V_ambx = V_ambx.to(device)
        ...
        if ep_tix % 1000 == 0:
            ...
            for itx in range(n_test_theta):
                ...
                for idx, sample in enumerate(dataloaders['train']):
                    ...
                    theta_ijx[:, args.learning_agent] = amb_theta
                    theta_ijx = theta_ijx.to(device)

                    V_hatx = q_network.forward(theta_ijx)
                    V_total += V_hatx.sum()
                writer.add_scalar("V_amb" + str(ep_tix), V_total / len(dataloaders['train']), itx)
