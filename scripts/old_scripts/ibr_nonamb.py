...
def run_iterative_best_response(other_vehicles,
    ...
    if load_log_dir:  # TODO:  This could use some cleanup
        ...
        _, _, _, xothers_actual_prev, uothers_actual_prev, _, = load_state(previous_all_file, params["n_other"], ignore_des=True)
    ...
    for i_mpc in range(i_mpc_start, params["n_mpc"]):
        ...
        for i_rounds_ibr in range(params["n_ibr"]):
            ...
            for response_i in vehicles_index_best_responders:
                ...
                # This could be a response_veh_info_ji (info about j, in i's reference frame)
                ...
                if params["save_ibr"]:
                    ...
                    save_state(log_dir + "data/" + "ibr_m%03di%03da%03d" % (i_mpc, i_rounds_ibr, response_i),
                               None, None, None, other_x_ibr_temp, other_u_ibr_temp,
                               other_xd_ibr_temp)
            ...
            all_other_x_ibr_g = [convert_to_global_units(np.zeros(6,1), x) for x in other_x_ibr_temp]
    
            if params["save_ibr"]:
                save_state(log_dir + "data/" + "ibr_m%03di%03d" % (i_mpc, i_rounds_ibr), None, None,
                           None, all_other_x_ibr_g, other_u_ibr_temp, other_xd_ibr_temp)
        ...
        if params["save_state"]:
            ...
            save_state(file_name, None, None, None, all_other_x_ibr_g, other_u_ibr_temp,
                       other_xd_ibr_temp)
            save_state(log_dir + "data/" + "all_%03d" % (i_mpc), None, None, None, xothers_actual, uothers_actual,
                       None, end_t=actual_t + params["number_ctrl_pts_executed"] + 1)
...
if __name__ == "__main__":
    ...
    _, xothers_actual = run_iterative_best_response(params, log_dir, args.load_log_dir, i_mpc_start,
                                                              all_other_x0, all_other_vehicles, world)
