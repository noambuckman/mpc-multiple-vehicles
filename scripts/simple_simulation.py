...
def initialize_simulation(params):
    ...
    return all_other_vehicles, all_other_x0, world, params
...
def run_simulation(log_dir, params):
    ...
    all_other_vehicles, all_other_x0, world, params = initialize_simulation(params)
    print("Initial positions")
    print(all_other_x0)
#     log_dir = None
    xothers_actual, others_actual = run_iterative_best_response(all_other_vehicles, world, all_other_x0, params, log_dir)

    all_trajectories = np.array(xothers_actual)
    all_control = np.array(others_actual)
...
experiment_random_seed = 719
current_sim_params = copy.deepcopy(default_params)
current_sim_params["p_cooperative"] = 0.5
current_sim_params["seed"] = [experiment_random_seed + ix for ix in range(1)]
