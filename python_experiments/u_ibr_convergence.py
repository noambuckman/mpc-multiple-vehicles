...
parser.add_argument('--n-agents', type=int, default=999, help="Max number of agents to plot")
...
for lidx, log_directory in enumerate(args.log_directories):
    ...
    # plt.show()
    fig1.savefig(log_directory + "plots/amb_conv_steering_i0" +"." + args.image_format, transparent=True)
    # plt.close()
    ...
    save_fig_custom(fig2, name = log_directory + "plots/amb_both_ctrl_i0" +"." + args.image_format)
    # plt.close()
    ...
    ax3.set_ylim([-.1, .1])
    # plt.show()
    save_fig_custom(fig3, name = log_directory + "plots/amb_convergence_steering" +"." + args.image_format)
    # plt.close()
    ...
    #     print(grid_i, grid_j)
        ag_id = veh_idxs[j]
        axs5[grid_i, grid_j].set_title("Ag %d"%ag_id)
