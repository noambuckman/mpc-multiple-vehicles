...
def run_iterative_best_response(params, log_dir, load_log_dir, i_mpc_start, amb_x0, all_other_x0, amb_MPC, all_other_MPC):
    ''' 
    ...
    actual_t = 0    
    xamb_actual, uamb_actual = np.zeros((6, params['n_mpc']*params["number_ctrl_pts_executed"] + 1)), np.zeros((2, params['n_mpc']*params["number_ctrl_pts_executed"])) 
    xothers_actual = [np.zeros((6, params['n_mpc']*params["number_ctrl_pts_executed"] + 1)) for i in range(params['n_other'])]
    uothers_actual = [np.zeros((2, params['n_mpc']*params["number_ctrl_pts_executed"])) for i in range(params['n_other'])]    
    ...
    for i_mpc in range(i_mpc_start, params['n_mpc']):
        ...
        if i_mpc > 0:
            ...
            all_other_x0_g = [cp.deepcopy(all_other_x_executed[i][:, params["number_ctrl_pts_executed"]]) for i in range(len(all_other_x_executed))]
        ...
        if i_mpc == 0:
            ...
            all_other_u_ibr, all_other_x_ibr, all_other_x_des_ibr = helper.extend_last_mpc_and_follow(previous_all_other_u_mpc, params["N"]-1, params["N"], all_other_MPC, all_other_x0, params, world)  
        else:
            # all_other_u_ibr, all_other_x_ibr, all_other_x_des_ibr = helper.extend_last_mpc_ctrl(all_other_u_mpc, params["number_ctrl_pts_executed"], N, all_other_MPC, all_other_x0)  
            all_other_u_ibr, all_other_x_ibr, all_other_x_des_ibr = helper.extend_last_mpc_and_follow(all_other_u_mpc, params["number_ctrl_pts_executed"], params["N"], all_other_MPC, all_other_x0, params, world)  
        ...
        for i_rounds_ibr in range(params['n_ibr']):    
            ...
            if i_mpc > 0: #TODO: What is going on here?
                u_warm_profiles["previous_mpc"] = np.concatenate((uamb_mpc[:, params["number_ctrl_pts_executed"]:], np.tile(uamb_mpc[:,-1:],(1, params["number_ctrl_pts_executed"]))), axis=1) ##    
            ...
            while solve_again and solve_number < params['k_max_solve_number']:
                ...
                with redirect_stdout(f):
                    if args.save_solver_input:
                        with open(log_dir + 'data/inputs_amb_mpc_%d_ibr_%d_s_%d.p'%(i_mpc, i_rounds_ibr, solve_number), 'wb') as fp:
    ...
    #             raise Exception("Test")
            ...
            if params["save_ibr"] == 1:
                file_name = log_dir + "data/"+'ibr_m%03di%03damb'%(i_mpc, i_rounds_ibr)
            ...
            for response_i in vehicles_index_best_responders:
                ...
                u_warm_profiles, ux_warm_profiles = mpc.generate_warm_u(params["N"], response_MPC, response_x0)            
                if i_mpc > 0:
                    u_warm_profiles["previous_mpc"] = np.concatenate((all_other_u_mpc[response_i][:, params["number_ctrl_pts_executed"]:], np.tile(all_other_u_mpc[response_i][:,-1:],(1, params["number_ctrl_pts_executed"]))),axis=1) ##    
                    x_warm, x_des_warm = response_MPC.forward_simulate_all(response_x0.reshape(6,1), u_warm_profiles["previous_mpc"])
                    ux_warm_profiles["previous_mpc"] = [u_warm_profiles["previous_mpc"], x_warm, x_des_warm]            
                if i_rounds_ibr > 0:
                ...
                while solve_again and solve_number < params['k_max_solve_number']:
                    ...
                    with redirect_stdout(f):
                        ...
                        if args.save_solver_input:
                            with open(log_dir + 'data/inputs_v%02d_mpc_%03d_ibr_%01d_s_%01d.p'%(response_i, i_mpc, i_rounds_ibr, solve_number), 'wb') as fp:
                ...
                if solve_again:
                    raise Exception("Reached maximum number of re-solves @ Veh %02d MPC Rd %d IBR %d.   Max Slack = %.05f > thresh %.05f"%(response_i, i_mpc, i_rounds_ibr, 
                                                                                                                            max_slack_ibr, params['k_max_slack']))
                if params["save_ibr"] == 1:
                    file_name = log_dir + "data/"+'ibr_m%03di%03da%03d'%(i_mpc, i_rounds_ibr, response_i)
            ...
            for j in range(len(all_other_x_ibr_g)):
                all_other_x_ibr_g[j][0,:] += amb_x0_g[0]        
            if params["save_ibr"] == 1:
                file_name = log_dir + "data/"+'ibr_m%03di%03d'%(i_mpc, i_rounds_ibr)            
                mpc.save_state(file_name, xamb_ibr_g, uamb_ibr, xamb_des_ibr, all_other_x_ibr_g, all_other_u_ibr, all_other_x_des_ibr)
        ...
        all_other_x_executed = [np.zeros(shape=(6, params["number_ctrl_pts_executed"]+1)) for i in range(params['n_other'])]
        all_other_u_executed = [np.zeros(shape=(2, params["number_ctrl_pts_executed"])) for i in range(params['n_other'])]

        ### SAVE EXECUTED MPC SOLUTION TO HISTORY
        xamb_executed, uamb_executed = xamb_mpc[:,:params["number_ctrl_pts_executed"]+1], uamb_mpc[:,:params["number_ctrl_pts_executed"]]
        xamb_actual[:, actual_t:actual_t+params["number_ctrl_pts_executed"]+1], uamb_actual[:, actual_t:actual_t+params["number_ctrl_pts_executed"]] = xamb_executed, uamb_executed
        for i in range(len(all_other_x_mpc)):
            all_other_x_executed[i], all_other_u_executed[i] = all_other_x_mpc[i][:,:params["number_ctrl_pts_executed"]+1], all_other_u_mpc[i][:,:params["number_ctrl_pts_executed"]]
            xothers_actual[i][:,actual_t:actual_t+params["number_ctrl_pts_executed"]+1], uothers_actual[i][:,actual_t:actual_t+params["number_ctrl_pts_executed"]] = all_other_x_executed[i], all_other_u_executed[i]
        ...
        file_name = log_dir + "data/"+'all_%03d'%(i_mpc)        
        mpc.save_state(file_name, xamb_actual, uamb_actual, None, xothers_actual, uothers_actual, None, end_t = actual_t+params["number_ctrl_pts_executed"]+1)
        
        mean_amb_v = np.mean(xamb_executed[4,:])
        im_dir = log_dir + '%03d/'%i_mpc
        os.makedirs(im_dir+"imgs/", exist_ok=True)    
        end_frame = actual_t + params["number_ctrl_pts_executed"] + 1
    ...
    final_t = actual_t+params["number_ctrl_pts_executed"]+1
    xamb_actual = xamb_actual[:,:actual_t+params["number_ctrl_pts_executed"]+1]
    for i in range(len(xothers_actual)):
        xothers_actual[i] = xothers_actual[i][:,:actual_t+params["number_ctrl_pts_executed"]+1]
...
if __name__ == "__main__":
    ...
    if args.load_log_dir is None:
        ...
        for i in range(len(all_other_MPC)):
            pickle.dump(all_other_MPC[i], open(log_dir + "data/mpcother%03d.p"%i,'wb'))
        pickle.dump(amb_MPC, open(log_dir + "data/mpcamb.p",'wb'))
        ...
        amb_MPC = pickle.load(open(log_dir + "data/mpcamb.p",'rb'))
        all_other_MPC = [pickle.load(open(log_dir + "data/mpcother%03d.p"%i,'rb')) for i in range(params["n_other"])]
    ...
    run_iterative_best_response(params, log_dir, args.load_log_dir, i_mpc_start, amb_x0, all_other_x0, amb_MPC, all_other_MPC)